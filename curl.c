...
int curl(RECV_BUF *out , int severVal,int n )
{
    ...
    sprintf(url, "http://ece252-%d.uwaterloo.ca:2520/image?img=%d",severVal,n);
    ...
    int seq_num = recv_buf.seq;
    out ->buf = recv_buf.buf;
    out ->max_size = recv_buf.max_size;
    out ->seq = seq_num;
    out ->size = recv_buf.size;
